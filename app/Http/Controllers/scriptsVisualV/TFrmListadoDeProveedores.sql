...
Begin VB.Form FrmListadoDeProveedores 
   BackColor       =   &H00FFFFC0&
   ...
   ClientTop       =   330
   ClientWidth     =   11865
   Icon            =   "FrmListadoDeProveedores.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin VB.CommandButton Cmdplano 
      ...
      Height          =   420
      Left            =   10035
      Picture         =   "FrmListadoDeProveedores.frx":46B2
      Style           =   1  'Graphical
      TabIndex        =   16
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CommandButton CmdTercero 
         Height          =   315
         Left            =   6840
         Picture         =   "FrmListadoDeProveedores.frx":6184
         Style           =   1  'Graphical
         TabIndex        =   17
         Top             =   495
         Width           =   315
   ...
   Begin MSComctlLib.ImageList ImageList1 
      ...
      BeginProperty Images {2C247F25-8591-11D1-B16A-00C0F0283628} 
         ...
         BeginProperty ListImage1 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "FrmListadoDeProveedores.frx":6746
            Key             =   ""
         EndProperty
         BeginProperty ListImage2 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "FrmListadoDeProveedores.frx":6C8A
            Key             =   ""
         EndProperty
         BeginProperty ListImage3 {2C247F27-8591-11D1-B16A-00C0F0283628} 
            Picture         =   "FrmListadoDeProveedores.frx":6D96
            Key             =   ""
         EndProperty
      EndProperty
   End
   ...
   Begin VB.Label LblTitle 
      BackColor       =   &H00A8753C&
      Caption         =   "      Analisis de Proveedores por Edades"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   12.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "FrmListadoDeProveedores"
...
Private Sub FGproveedor_AfterCollapse(ByVal Row As Long, ByVal State As Integer)
        If Row <> 0 Then
               If State = 0 Then
                    FGproveedor.ColHidden(Datos.Fecha) = False
                    FGproveedor.ColHidden(Datos.FechaVen) = False
                    FGproveedor.ColHidden(Datos.Dias) = False
               Else
                    FGproveedor.ColHidden(Datos.Fecha) = True
                    FGproveedor.ColHidden(Datos.FechaVen) = True
                    FGproveedor.ColHidden(Datos.Dias) = True
               End If
        Else
...
Private Sub DGRILLA()
        ...
        With FGproveedor
             ...
             For K = Datos.Total To Datos.MDeuda
                .ColFormat(K) = "#,###.00"
                .ColWidth(K) = 1200
             Next K
        ...
        With FGNovedades
             ...
             .TextMatrix(0, Novedad.ConceptoCxP) = "Concepto"
             .FocusRect = flexFocusRaised
             .OutlineBar = flexOutlineBarComplete
             .Cell(flexcpAlignment, 0, 0, 0, .Cols - 1) = flexAlignCenterCenter
             .AllowUserResizing = flexResizeColumns
        ...
        Exit Sub
sALIDA: Call MensajeAdministrador("DGRILLA")
End Sub
...
Private Sub Imprimir_Informe(vTipo As Integer)
        With FGproveedor
             ...
             M = -1: Ax = Empty
             N = 0
             If Trim(TxtTercero.Text) <> Empty Then K = 1 Else K = 1
             If vTipo = 1 Then
                 For i = K To .Rows - 1
             ...
             Mx = "ANALISIS DE PROVEEDORES POR EDADES"
             ...
             X = Inf + "AnalisisDeProveedores.rpt"
             RepCartera.ReportFileName = X
             RepCartera.Destination = crptToWindow
             RepCartera.WindowTitle = "LISTADO DE ANALISIS DE PROVEEDORES POR EDADES"
...
Private Sub Deuda_Proveedores()
        On Error GoTo sALIDA
        
        Dim vDevolucionSinSoporte As Boolean, vReciboSinSoporte As Boolean, Tactual As Integer
        ...
        With FGdatos
             ...
             If Trim(TxtTercero.Text) <> Empty Then
                 vFiltro = " AND PrvIdStProveedor = '" & TxtTercero.Text & "' "
             End If
             ...
             A = Empty
             Sql = "SELECT A.*,B.CliStNombreCliente FROM PROVEEDORES A " & _
                   "INNER JOIN CLIENTES B ON (A.PrvIdStEmpresa = B.CliEmpr AND A.PrvIdStProveedor = B.CliNit) " & _
                   "WHERE PrvIdStEmpresa = '" & Empresa & "' AND PrvIdStPeriodoAplicacion = '1' AND PrvDaFechaCompra <= '" & Format(DTfechaI.Value, FormatoFecha) & "' " & vFiltro & _
             ...
             If Buscar_Res(rdoRs, Sql) = BDOk Then
                While Not rdoRs.EOF
                    .AddItem ""
                    .TextMatrix(.Rows - 1, SaldoCx.Cedula) = Trim("" & rdoRs!PrvIdStProveedor)
                    .TextMatrix(.Rows - 1, SaldoCx.Nombre) = Trim("" & rdoRs!CliStNombreCliente)
                    .TextMatrix(.Rows - 1, SaldoCx.IDconcepto) = Trim("" & rdoRs!PrvIdInConceptoProveedores)
                    .TextMatrix(.Rows - 1, SaldoCx.Valor) = Round(Val("" & rdoRs!PrvInValor), 2)
                    .TextMatrix(.Rows - 1, SaldoCx.Fecha) = Trim("" & rdoRs!PrvDaFechaCompra)
                    .TextMatrix(.Rows - 1, SaldoCx.Dias) = Val("" & rdoRs!PrvInDias)
                    If Trim("" & rdoRs!PrvStFactura) <> Empty Then
                         .TextMatrix(.Rows - 1, SaldoCx.Factura) = Trim("Factura No. " & rdoRs!PrvStFactura)
                    Else
                         .TextMatrix(.Rows - 1, SaldoCx.Factura) = Trim("Nota No. " & rdoRs!PrvIdStDocumento)
                    End If
                    
                    vFecha = Trim("" & rdoRs!PrvDaFechaCompra)
                    If Val("" & rdoRs!PrvInDias) > 0 Then vFecha = vFecha + Val("" & rdoRs!PrvInDias)
                    .TextMatrix(.Rows - 1, SaldoCx.FFinal) = vFecha
                    .TextMatrix(.Rows - 1, SaldoCx.DiaFinal) = DateDiff("d", vFecha, Me.DTfechaI.Value)
                    
                    X = Trim("" & rdoRs!PrvIdStProveedor) & "p" & Trim("" & rdoRs!PrvIdStComprobante) & "p" & Trim("" & rdoRs!PrvIdStDocumento) & "p" & Trim("" & rdoRs!PrvIdInConceptoProveedores)
                    Y = Trim("" & rdoRs!PrvIdStProveedor) & "p" & Trim("" & rdoRs!PrvIdStComprobante) & "p" & Trim("" & rdoRs!PrvStFactura) & "p" & Trim("" & rdoRs!PrvIdInConceptoProveedores)
                    vColCartera.Add .Rows - 1, X
                    If Trim("" & rdoRs!PrvStFactura) <> Empty Then vColDevVenta.Add .Rows - 1, Y
             ...
             If Trim(vParametros(56)) <> Empty Then 'Si hay una fecha de corte por parametros
                  Sql = "SELECT A.*,B.MprIdStComprobanteCompra,B.MprIdStDocumentoCompra,B.MprInValor,B.MprIdInConceptoProveedor,B.MprIdStProveedor FROM PAGOS A " & _
                        ...
                        "WHERE (A.PprIdStEmpresa = '" & Empresa & "' AND (A.PprIdStTipoComprobante = '" & vTipoComprobante(1) & "' OR A.PprIdStTipoComprobante = '" & vTipoComprobante(3) & "' OR A.PprIdStTipoComprobante = '" & vTipoComprobante(10) & "') AND " & _
                               "PprDaFechaPago BETWEEN  '" & vFechaIni & "' AND '" & Format(DTfechaI.Value, FormatoFecha) & "'  AND PprStEstadoPago ='" & EAprobado & "'" & vFiltro & ") "
             Else
                  Sql = "SELECT A.*,B.MprIdStComprobanteCompra,B.MprIdStDocumentoCompra,B.MprInValor,B.MprIdInConceptoProveedor,B.MprIdStProveedor FROM PAGOS A " & _
                        "INNER JOIN MOVPAGOS B ON (A.PprIdStEmpresa = B.MprIdStEmpresa AND A.PprIdStComprobante = B.MprIdStComprobante AND A.PprIdInDocumento = B.MprIdInDocumento) " & _
                        "WHERE (A.PprIdStEmpresa = '" & Empresa & "' AND (A.PprIdStTipoComprobante = '" & vTipoComprobante(1) & "' OR A.PprIdStTipoComprobante = '" & vTipoComprobante(3) & "' OR A.PprIdStTipoComprobante = '" & vTipoComprobante(10) & "') AND " & _
             ...
             If Buscar_Res(rdoRs, Sql) = BDOk Then
                While Not rdoRs.EOF
                      If Trim("" & rdoRs!MprIdStProveedor) <> Empty Then
                          M = Trim("" & rdoRs!MprIdStProveedor) & "p" & Trim("" & rdoRs!MprIdStComprobanteCompra) & "p" & Trim("" & rdoRs!MprIdStDocumentoCompra) & "p" & Trim("" & rdoRs!MprIdInConceptoProveedor)
                          P = Trim("" & rdoRs!MprIdStProveedor)
                      Else
                          M = Trim("" & rdoRs!PprIdStProveedor) & "p" & Trim("" & rdoRs!MprIdStComprobanteCompra) & "p" & Trim("" & rdoRs!MprIdStDocumentoCompra) & "p" & Trim("" & rdoRs!MprIdInConceptoProveedor)
                          P = Trim("" & rdoRs!PprIdStProveedor)
                      End If
                      On Error Resume Next
                      T = "-1"
                      T = vColCartera(M)
                      If T <> "-1" Then
                           .TextMatrix(T, SaldoCx.Valor) = Val(.TextMatrix(T, SaldoCx.Valor)) - Round(Val("" & rdoRs!MprInValor), 2)
                      Else
                          With FGNovedades
                               .AddItem ""
                               .TextMatrix(.Rows - 1, Novedad.Cliente) = P
                               .TextMatrix(.Rows - 1, Novedad.Comprobante) = Trim("" & rdoRs!MprIdStComprobanteCompra)
                               .TextMatrix(.Rows - 1, Novedad.Fecha) = Trim("" & rdoRs!PprDaFechaPago)
                               .TextMatrix(.Rows - 1, Novedad.numero) = Trim("" & rdoRs!MprIdStDocumentoCompra)
                               .TextMatrix(.Rows - 1, Novedad.ConceptoCxP) = Trim("" & rdoRs!MprIdInConceptoProveedor)
                               .TextMatrix(.Rows - 1, Novedad.vValor) = Round(Val("" & rdoRs!MprInValor), 2)
                               .TextMatrix(.Rows - 1, Novedad.ComprobanteDoc) = Trim("" & rdoRs!PprIdStComprobante)
                               .TextMatrix(.Rows - 1, Novedad.ConsecutivoDoc) = Trim("" & rdoRs!PprIdInDocumento)
                               .TextMatrix(.Rows - 1, Novedad.Descri) = "EGRESO O ND PROVEEDORES"
                          End With
            ...
            '*****busqueda de pagos por caja menor*****
             If Trim(TxtTercero.Text) <> Empty Then
                 vFiltro = " AND CmeIdStProveedor = '" & TxtTercero.Text & "' "
             End If
             If Trim(vParametros(56)) <> Empty Then 'Si hay una fecha de corte por parametros
                  Sql = "SELECT A.*,B.McmIdStComprobanteCompra,B.McmIdStDocumentoCompra,B.McmInValor,B.McmIdInConceptoProveedor FROM CAJAMENOR A " & _
                        ...
                        "WHERE (A.CmeIdStEmpresa = '" & Empresa & "' AND " & _
                               "CmeDaFechaDoc BETWEEN  '" & vFechaIni & "' AND '" & Format(DTfechaI.Value, FormatoFecha) & "'  AND (CmeStEstadoRegistro is null or CmeStEstadoRegistro = '" & EAprobado & "') " & vFiltro & ")"
             Else
                  Sql = "SELECT A.*,B.McmIdStComprobanteCompra,B.McmIdStDocumentoCompra,B.McmInValor,B.McmIdInConceptoProveedor FROM CAJAMENOR A " & _
                        "INNER JOIN MOVCAJAMENOR B ON (A.CmeIdStEmpresa = B.McmIdStEmpresa AND A.CmeIdInCaja = B.McmIdInCaja AND A.CmeStComprobante = B.McmIdStComprobante AND A.CmeIdInDocumento = B.McmIdInDocumento) " & _
                        "WHERE (A.CmeIdStEmpresa = '" & Empresa & "' AND " & _
                               "CmeDaFechaDoc <= '" & Format(DTfechaI.Value, FormatoFecha) & "'  AND (CmeStEstadoRegistro is null or CmeStEstadoRegistro = '" & EAprobado & "') " & vFiltro & ") "
             End If
             If Buscar_Res(rdoRs, Sql) = BDOk Then
                While Not rdoRs.EOF
                      M = Trim("" & rdoRs!CmeIdStProveedor) & "p" & Trim("" & rdoRs!McmIdStComprobanteCompra) & "p" & Trim("" & rdoRs!McmIdStDocumentoCompra) & "p" & Trim("" & rdoRs!McmIdInConceptoProveedor)
                      On Error Resume Next
                      T = "-1"
                      T = vColCartera(M)
                      If T <> "-1" Then
                           .TextMatrix(T, SaldoCx.Valor) = Val(.TextMatrix(T, SaldoCx.Valor)) - Round(Val("" & rdoRs!McmInValor), 2)
                      Else
                          With FGNovedades
                               .AddItem ""
                               .TextMatrix(.Rows - 1, Novedad.Cliente) = Trim("" & rdoRs!CmeIdStProveedor)
                               .TextMatrix(.Rows - 1, Novedad.Comprobante) = Trim("" & rdoRs!McmIdStComprobanteCompra)
                               .TextMatrix(.Rows - 1, Novedad.Fecha) = Trim("" & rdoRs!CmeDaFechaDoc)
                               .TextMatrix(.Rows - 1, Novedad.numero) = Trim("" & rdoRs!McmIdStDocumentoCompra)
                               .TextMatrix(.Rows - 1, Novedad.ConceptoCxP) = Trim("" & rdoRs!McmIdInConceptoProveedor)
                               .TextMatrix(.Rows - 1, Novedad.vValor) = Round(Val("" & rdoRs!McmInValor), 2)
                               .TextMatrix(.Rows - 1, Novedad.ComprobanteDoc) = Trim("" & rdoRs!CmeStComprobante)
                               .TextMatrix(.Rows - 1, Novedad.ConsecutivoDoc) = Trim("" & rdoRs!CmeIdInDocumento)
                               .TextMatrix(.Rows - 1, Novedad.Descri) = "EGRESO DE CAJA MENOR"
                          End With
                          vReciboSinSoporte = True
                      End If
                      rdoRs.MoveNext
            ...
            '********* Devoluciones en compras **************************************************
             ...
             End If
             If Trim(vParametros(56)) <> Empty Then 'Si hay una fecha de corte por parametros
                  Sql = "SELECT ComIdStDocumentoSoporte,ComIdStProveedor,ComIdStComprobante,ComIdStDocumento,ComDaFechaFactura,ComInValor,ComInValorIva,ComInValorDescuento,ComStRetencionAsumida,ComInValorRetencion,ComIdStComprobanteOrdenCompra,ComIdInConceptoProveedores " & _
                        "FROM COMPRAS A " & _
                        "WHERE (A.ComIdStEmpresa = '" & Empresa & "' AND A.ComIdStTipoComprobante = '" & vTipoComprobante(7) & "' AND " & _
                               "ComDaFechaFactura BETWEEN  '" & vFechaIni & "' AND '" & Format(DTfechaI.Value, FormatoFecha) & "' AND ComStEstadoCompra = '" & EAprobado & "'" & vFiltro & ") "
             Else
                  Sql = "SELECT ComIdStDocumentoSoporte,ComIdStProveedor,ComIdStComprobante,ComIdStDocumento,ComDaFechaFactura,ComInValor,ComInValorIva,ComInValorDescuento,ComStRetencionAsumida,ComInValorRetencion,ComIdStComprobanteOrdenCompra,ComIdInConceptoProveedores " & _
                        "FROM COMPRAS A " & _
                        "WHERE (A.ComIdStEmpresa = '" & Empresa & "' AND A.ComIdStTipoComprobante = '" & vTipoComprobante(7) & "' AND " & _
                              "ComDaFechaFactura <= '" & Format(DTfechaI.Value, FormatoFecha) & "' AND ComStEstadoCompra = '" & EAprobado & "'" & vFiltro & ") "
             End If
             If Buscar_Res(rdoRs, Sql) = BDOk Then
                While Not rdoRs.EOF
                      If Trim("" & rdoRs!ComIdStComprobanteOrdenCompra) = Empty Then
                          c = vParametros(1)
                      Else
                          c = Trim("" & rdoRs!ComIdStComprobanteOrdenCompra)
                      End If
                      M = Trim("" & rdoRs!ComIdStProveedor) & "p" & c & "p" & Trim("" & rdoRs!ComIdStDocumentoSoporte) & "P" & Trim("" & rdoRs!ComIdInConceptoProveedores)
                      On Error Resume Next
                      T = "-1"
                      T = vColDevVenta(M)
                      If T <> "-1" Then
                           If Val("" & rdoRs!ComStRetencionAsumida) = 1 Then
                                Ny = Val("" & rdoRs!ComInValor) + Val("" & rdoRs!ComInValorIva) - Val("" & rdoRs!ComInValorDescuento)
                           Else
                               Ny = Val("" & rdoRs!ComInValor) + Val("" & rdoRs!ComInValorIva) - Val("" & rdoRs!ComInValorDescuento) - Val("" & rdoRs!ComInValorRetencion)
                           End If
                           .TextMatrix(T, SaldoCx.Valor) = Val(.TextMatrix(T, SaldoCx.Valor)) - Round(Ny, 2)
                      Else
                          With FGNovedades
                              .AddItem ""
                              .TextMatrix(.Rows - 1, Novedad.Cliente) = Trim("" & rdoRs!ComIdStProveedor)
                              .TextMatrix(.Rows - 1, Novedad.Comprobante) = c
                              .TextMatrix(.Rows - 1, Novedad.Fecha) = Trim("" & rdoRs!ComDaFechaFactura)
                              .TextMatrix(.Rows - 1, Novedad.ConceptoCxP) = Trim("" & rdoRs!ComIdInConceptoProveedores)
                              .TextMatrix(.Rows - 1, Novedad.numero) = Trim("" & rdoRs!ComIdStDocumentoSoporte)
                              .TextMatrix(.Rows - 1, Novedad.vValor) = Round(Ny, 2)
                              .TextMatrix(.Rows - 1, Novedad.ComprobanteDoc) = Trim("" & rdoRs!ComIdStComprobante)
                              .TextMatrix(.Rows - 1, Novedad.ConsecutivoDoc) = Trim("" & rdoRs!ComIdStDocumento)
                              .TextMatrix(.Rows - 1, Novedad.Descri) = "DEVOLUCIONES EN COMPRA"
                          End With
            ...
            '********************************************************************************************
             If vDevolucionSinSoporte = True Or vReciboSinSoporte = True Then
                 If vReciboSinSoporte = True Then
                      MsgBox "Se�or usuario hay pagos sin soporte", vbInformation + vbOKOnly, "Atenci�n"
                 Else
                     MsgBox "Se�or usuario hay Devoluciones de compras sin soporte", vbInformation + vbOKOnly, "Atenci�n"
                 End If
                 Progres.Visible = False
                 Mx = Format(FechaServidor, "yyyymmdd")
                 FGNovedades.SaveGrid App.Path & "\NOVEDADES DE CxP-" & Mx & ".xls", flexFileExcel, 3
                 MsgBox "Se�or usuario se genero el archivo llamado NOVEDADES DE CxP-" & Mx & ".xls", vbInformation + vbOKOnly, "Atenci�n"
                 Exit Sub
             End If
           ...
           '*********************************************************************************************************************************
             ...
             If FGdatos.Rows > 1 Then
                   ...
                   FGproveedor.Rows = 1: A = Empty
                   If Trim(TxtTercero.Text) = Empty Then
                        FGproveedor.AddItem "": vIni = FGproveedor.Rows - 1
                        FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Cliente) = "TOTAL"
                        FGproveedor.Cell(flexcpForeColor, FGproveedor.Rows - 1, 0, FGproveedor.Rows - 1, FGproveedor.Cols - 1) = ColorNivelDos
                        Call Agrupar_Niveles(FGproveedor, FGproveedor.Rows - 1, 1)
                   Else
                        vIni = 0
                   End If
                   ...
                   For i = 1 To .Rows - 1
                       If A <> Trim(.TextMatrix(i, SaldoCx.Cedula)) Then
                            FGproveedor.AddItem ""
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Cedula) = Trim(.TextMatrix(i, SaldoCx.Cedula))
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Cliente) = Trim(.TextMatrix(i, SaldoCx.Nombre))
                            FGproveedor.Cell(flexcpForeColor, FGproveedor.Rows - 1, 0, FGproveedor.Rows - 1, FGproveedor.Cols - 1) = vbBlue
                            Call Agrupar_Niveles(FGproveedor, FGproveedor.Rows - 1, 1)
                            X1 = FGproveedor.Rows - 1
                       End If
                       ...
                       If Round(Val(.TextMatrix(i, SaldoCx.Valor)), 2) > 0 Then
                            ...
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Fecha) = Trim(.TextMatrix(i, SaldoCx.Fecha))
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.FechaVen) = Trim(.TextMatrix(i, SaldoCx.FFinal))
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Cliente) = Trim(.TextMatrix(i, SaldoCx.Factura))
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Concepto) = Trim(.TextMatrix(i, SaldoCx.IDconcepto))
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Dias) = Trim(.TextMatrix(i, SaldoCx.Dias))
                            FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Total) = Val(.TextMatrix(i, SaldoCx.Valor))
                            FGproveedor.TextMatrix(X1, Datos.Total) = Val(FGproveedor.TextMatrix(X1, Datos.Total)) + Val(.TextMatrix(i, SaldoCx.Valor))
                            vDias = Val(.TextMatrix(i, SaldoCx.DiaFinal))
                            If vDias <= 0 Then
                                FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.vDeuda) = Val(.TextMatrix(i, SaldoCx.Valor))
                                FGproveedor.TextMatrix(X1, Datos.vDeuda) = Val(FGproveedor.TextMatrix(X1, Datos.vDeuda)) + Val(.TextMatrix(i, SaldoCx.Valor))
                            Else
                                If vDias > 0 And vDias <= 30 Then
                                     FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Deuda30) = Val(.TextMatrix(i, SaldoCx.Valor))
                                     FGproveedor.TextMatrix(X1, Datos.Deuda30) = Val(FGproveedor.TextMatrix(X1, Datos.Deuda30)) + Val(.TextMatrix(i, SaldoCx.Valor))
                                Else
                                     If vDias > 30 And vDias <= 60 Then
                                          FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Deuda60) = Val(.TextMatrix(i, SaldoCx.Valor))
                                          FGproveedor.TextMatrix(X1, Datos.Deuda60) = Val(FGproveedor.TextMatrix(X1, Datos.Deuda60)) + Val(.TextMatrix(i, SaldoCx.Valor))
                                     Else
                                          If vDias > 60 And vDias <= 90 Then
                                                FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Deuda90) = Val(.TextMatrix(i, SaldoCx.Valor))
                                                FGproveedor.TextMatrix(X1, Datos.Deuda90) = Val(FGproveedor.TextMatrix(X1, Datos.Deuda90)) + Val(.TextMatrix(i, SaldoCx.Valor))
                                          Else
                                               If vDias > 90 And vDias <= 180 Then
                                                    FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Deuda180) = Val(.TextMatrix(i, SaldoCx.Valor))
                                                    FGproveedor.TextMatrix(X1, Datos.Deuda180) = Val(FGproveedor.TextMatrix(X1, Datos.Deuda180)) + Val(.TextMatrix(i, SaldoCx.Valor))
                                               Else
                                                    If vDias > 180 And vDias <= 360 Then
                                                        FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.Deuda360) = Val(.TextMatrix(i, SaldoCx.Valor))
                                                        FGproveedor.TextMatrix(X1, Datos.Deuda360) = Val(FGproveedor.TextMatrix(X1, Datos.Deuda360)) + Val(.TextMatrix(i, SaldoCx.Valor))
                                                    Else
                                                        FGproveedor.TextMatrix(FGproveedor.Rows - 1, Datos.MDeuda) = Val(.TextMatrix(i, SaldoCx.Valor))
                                                        FGproveedor.TextMatrix(X1, Datos.MDeuda) = Val(FGproveedor.TextMatrix(X1, Datos.MDeuda)) + Val(.TextMatrix(i, SaldoCx.Valor))
                                                    End If
                                               End If
                                          End If
                                     End If
                            ...
                            If vIni > 0 Then
                                For K = Datos.Total To Datos.MDeuda
                                   FGproveedor.TextMatrix(vIni, K) = Val(FGproveedor.TextMatrix(vIni, K)) + Val(FGproveedor.TextMatrix(FGproveedor.Rows - 1, K))
                                Next K
                            End If
                       End If
                   ...
                   For i = 1 To FGproveedor.Rows - 1
                       If Trim(Me.FGproveedor.TextMatrix(i, Datos.Cedula)) = "890800788" Then
                         f999 = 0
                       End If
                       
                       If i < Me.FGproveedor.Rows Then
                            If Val(Me.FGproveedor.TextMatrix(i, Datos.Total)) = 0 Then
                                  FGproveedor.RemoveItem (i)
                                  i = i - 1
                            End If
                       End If
                   Next i
                        
                   If FGproveedor.Rows > 1 Then
                       FGproveedor.Select 1, Datos.Cliente: FGproveedor.SetFocus
                   End If
             Else
             ...
             Progres.Value = Progres.Value + 1
             Progres.Visible = False
             FGproveedor.AutoSize Datos.Total, FGproveedor.Cols - 1
             FGproveedor.AllowUserResizing = flexResizeColumns
             FGproveedor.Outline (1)
             
             
        End With
        Exit Sub
sALIDA: Call MensajeAdministrador("Documento Interno:" & X & Chr(13) & "Factura:" & Y)
FGdatos.Redraw = flexRDDirect
End Sub

Private Sub PROVEEDORES_Anual()
        With Me.FGproveedor
        Sql = "SELECT A.*,B.CliStNombreCliente FROM CARTERAANUAL A " & _
                   "INNER JOIN CLIENTES B ON (A.PrvIdStEmpresa = B.CliEmpr AND A.PrvIdStProveedor = B.CliNit) " & _
                   "WHERE PrvIdStEmpresa = '" & Empresa & "' AND CarIdInPeriodoAno = " & Ini & " AND CarIdInPeriodoMes = 12 AND " & _
                   "PrvIdStPeriodoAplicacion = '1' AND " & vFiltro & _
                   "(PrvIdStComprobante = '" & vParametros(3) & "' Or PrvIdStComprobante = '" & vParametros(12) & "') AND PrvInValor > PrvInAbono " & _
             ...
             If Buscar_Res(rdoRs, Sql) = BDOk Then
                While Not rdoRs.EOF
                      ...
                      .TextMatrix(.Rows - 1, SaldoCx.Cedula) = Trim("" & rdoRs!PrvIdStProveedor)
                      .TextMatrix(.Rows - 1, SaldoCx.Nombre) = Trim("" & rdoRs!CliStNombreCliente)
                      .TextMatrix(.Rows - 1, SaldoCx.Valor) = Val("" & rdoRs!PrvInValor) - Val("" & rdoRs!PrvInAbono)
                      .TextMatrix(.Rows - 1, SaldoCx.Fecha) = Trim("" & rdoRs!PrvDaFechaCompra)
                      .TextMatrix(.Rows - 1, SaldoCx.Dias) = Val("" & rdoRs!PrvInDias)
                      If Trim("" & rdoRs!PrvStFactura) <> Empty Then
                           .TextMatrix(.Rows - 1, SaldoCx.Factura) = Trim("Factura No. " & rdoRs!PrvStFactura)
